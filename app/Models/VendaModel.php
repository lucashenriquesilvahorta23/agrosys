...
class VendaModel extends Model{
    ...
    function getVendas(){
        $builder = $this->db->table('procedimento_venda');
    ...
    function getDetalheProcedimento($id){
        $builder = $this->db->table('procedimento_venda_animal AS a');
        ...
        $builder->where('a.id_procedimento_venda', $id);
    ...
    public function getQuantidadeAnimaisPorProcedimento()
    {
        ...
        $builder->select('pv.descricao AS procedimento, COUNT(pva.id_procedimento_venda_animal) AS quantidade, sum(valor) as total')
                ->join('procedimento_venda AS pv', 'pva.id_procedimento_venda = pv.id_procedimento_venda', 'INNER')
                ->groupBy('pva.id_procedimento_venda')
                ->orderBy('quantidade', 'DESC');

        return $builder->get()->getResult();
    }

    public function getAnimaisMaisPesadosPorProcedimento($id_procedimento_venda)
    {
        ...
        $builder->select('pva.identificacao_animal, pva.peso, pva.valor')
                ->where('pva.id_procedimento_venda', $id_procedimento_venda)
